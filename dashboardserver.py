...
@login_required
def api_comparativo():
    """Get manager comparison data - groups restaurants by manager"""
    try:
        ...
        # Group restaurants by manager
        managers_map = {}
        
        for restaurant in restaurants:
            # Extract manager name (handle different formats)
            manager_name = (
                restaurant.get('manager') or
                restaurant.get('merchantManager', {}).get('name') if isinstance(restaurant.get('merchantManager'), dict) else restaurant.get('merchantManager') or
                'Sem Gestor'
            )
            
            # Initialize manager entry if doesn't exist
            if manager_name not in managers_map:
                managers_map[manager_name] = {
                    'name': manager_name,
                    'restaurants': [],
                    'totalRevenue': 0,
                    'totalOrders': 0,
                    'trend': 0  # Will calculate average trend
                }
            
            # Extract metrics (handle different formats)
            metrics = restaurant.get('metrics', {})
            
            # Revenue - try different field names
            revenue = (
                metrics.get('valor_bruto') or
                metrics.get('totals', {}).get('valorBruto') or
                restaurant.get('totalRevenue') or
                0
            )
            
            # Orders - try different field names  
            orders = (
                metrics.get('total_pedidos') or
                metrics.get('totals', {}).get('vendas') or
                restaurant.get('totalOrders') or
                0
            )
            
            # Trend - try different field names
            trend = (
                metrics.get('trends', {}).get('vendas') or
                restaurant.get('trend') or
                0
            )
            
            # Add to manager's totals
            managers_map[manager_name]['totalRevenue'] += revenue
            managers_map[manager_name]['totalOrders'] += orders
            managers_map[manager_name]['restaurants'].append({
                'id': restaurant.get('id'),
                'name': restaurant.get('name'),
                'revenue': revenue,
                'orders': orders,
                'trend': trend,
                'avgTicket': revenue / orders if orders > 0 else 0
            })
        
        # Calculate average trend for each manager
        for manager_data in managers_map.values():
            if manager_data['restaurants']:
                total_trend = sum(r['trend'] for r in manager_data['restaurants'])
                manager_data['trend'] = total_trend / len(manager_data['restaurants'])
        ...
        print(f"❌ Error in comparativo API: {e}")
        traceback.print_exc()
        return jsonify({
            'success': False,
            'error': str(e),
